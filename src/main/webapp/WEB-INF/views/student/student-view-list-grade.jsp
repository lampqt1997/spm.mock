		...
		<div class="row">
			...
			<div class="col-11">
				<div class="row p-2 mt-3">
					...
					<div class="col-4">
						<p class="text-secondary h3">Các bài test hiện có trong hệ thống</p>	
						<table class="table table-inverse mt-5">
							...
							<tbody>
								<%int i=1; %>
								<c:forEach var="j" items="${tests }">
								<tr>
									<td><%=i++ %></td>
									<td>
										<a href="/spm.mock/handler/st-view-grade/<c:out value="${userId}"></c:out>/<c:out value="${j.getId()}"></c:out>" class="btn btn-link text-info"> ${j.getTname() }</a>
									</td>	
								</tr>
								</c:forEach>
							</tbody>
