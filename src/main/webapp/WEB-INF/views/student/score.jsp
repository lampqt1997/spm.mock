					...
					<div class="col-3 ">
						<div class="card p-5">
							<div class="card-block">
								...
								<p class="text-left ">Bạn vừa làm bài test: 
									<span class="test-name badge badge-info float-right h1">
										<c:out value="${score.getNameTest() }"></c:out>
									</span>
								</p>
								<p class="text-left">Số câu hỏi trong bài test: 
									<span class="test-count-question badge badge-secondary float-right" >
										<c:out value="${score.getCountQuestion() }"></c:out>
									</span>
								</p>
								<p class="text-left">Số câu trả lời của bạn: 
									<span class="test-count-answer badge badge-secondary float-right">
										<c:out value="${score.getCountAnswer() }"></c:out>										
									</span>
								</p>
								<p class="text-left">Số câu trả lời đúng: 										
									<span class="test-answer-true badge badge-secondary float-right">
										<c:out value="${score.getTrueAnswer() }"></c:out>
									</span>
								</p>
								<p class="text-left">Thời gian nộp: 								
									<span class="test-time badge badge-secondary float-right">
										<% Date d = new Date(System.currentTimeMillis()); %>
										<%=d %>
									</span>											
								</p>
								<p class="text-left">Điểm: 
									<span class="test-score badge badge-danger float-right">
										<c:out value="${String.format('%.2g%n',score.getScore())}"></c:out>
									</span>
								</p>										
							</div>
						</div>
